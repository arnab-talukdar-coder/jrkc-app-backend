...
export const clockIn = async (req, res) => {
  try {
    ...
    const queryParams = [];
    if (employeeId) {
      const safeId = String(employeeId).replace(/[.*+?^$\\{\\}()|[\\]\\\\]/g, '\\    const queryParams = [];
    ...
    const finalEmployeeId = emp.id;');
      queryParams.push({ id: new RegExp('^' + safeId + '
...
, 'i') });
    ...
    let emp = null;
    try {
      emp = await Employee.findOne({ $or: queryParams });
    } catch (e) {}

    if (!emp) {
      emp = memEmployees.find(e => {
        if (employeeId && String(e.id).toLowerCase() === String(employeeId).toLowerCase()) return true;
        if (email && e.email.toLowerCase() === email.toLowerCase().trim()) return true;
        return false;
      });
    }
    ...
    // Normalize employeeId to the one from DB or memory
    const finalEmployeeId = emp.id;
